class SupplementsController < ApplicationController
  ...
  # GET /supplements
  def index
    supplements_found = SupplementIngr.find(params[:supplement_ingr_id]).supplements.where(shopping_site: params[:shopping_site])
    @supplements = SupplementSerializer.new(supplements_found.paginate(page: params[:page], per_page: 12)).serialized_json
    #헤더에 total 갯수 넣어줌
