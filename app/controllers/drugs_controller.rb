class DrugsController < ApplicationController
  ...
  def show
    ...
    if params[:sub_user_id].present?
      ...
      sub_user_current_drug_ids = sub_user.current_drugs.ids
      if sub_user_current_drug_ids.include?(@drug.id)
        @data.merge!(:currently_taking => {:current_drug_id => sub_user_current_drug_ids[sub_user.current_drug_ids.index(@drug.id)] })
      else
